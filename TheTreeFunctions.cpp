...
nodeOfStuck* head=NULL;

void inserttostack(nodeOfStuck* &head,node* x){
	nodeOfStuck* h=new nodeOfStuck;
	h->Lptr=head;
	h->Rptr=NULL;
	head->Rptr=h;
	h->info= x;
...
void Createstack(nodeOfStuck* &head,node* x){
	nodeOfStuck* h=new nodeOfStuck;
	h->Lptr=NULL;
	h->Rptr=NULL;
	h->info= x;
...
do{
	...
	if(h->Rptr!=NULL&&h->Rptr!=head->Rptr->info){
